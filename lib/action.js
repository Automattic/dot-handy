...
const runPrepsAndAction_ = async ( runEnv, preps, primaryAction, accum ) => {
	if ( preps.length == 0 ) {
...
const runActions_ = async ( runEnv, actions, accum ) => {
	if ( actions.length == 0 ) {
		return accum;
	}
	...
	const prepsAndActionResult = await runPrepsAndAction_(
		runEnv, actions.slice( 0, prepEnd ), firstNonPrepAction, accum
	);
...
/**
 * The main public function for running actions.
 *
 * @param {object} runEnv the browser automation environment in shape of { browser, context, page, config }
 * @param {array} actions array of actions
 *
 * @return {object} a chain-accumulated result of running the given sequence of actions.
 */
const runActions = async ( runEnv, actions ) => {
	...
	return await runActions_( runEnv, actions, {} );
};
